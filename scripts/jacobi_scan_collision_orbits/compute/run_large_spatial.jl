...
C_values = collect(2.90:0.01:3.20)
N_ETA = 10
N_PHI = 44
N_PSI = 44
tspan  = 20pi   # fictitious time for backward propagation

cb_escape = ContinuousCallback((u, _, _) -> u[1]^2 + u[2]^2 + u[3]^2 + u[4]^2 - 0.22, terminate!)
...
const DATA_PATH = joinpath(DATA_DIR, "large_jacobi_scan_spatial.jld2")
mkpath(DATA_DIR)
save(DATA_PATH,
     "cart",      all_cart,
     "ks",        all_ks,
